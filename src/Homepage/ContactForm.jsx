...
import './ContactForm.module.css'; // CSS Module import
...
const ContactForm = ({ course, formData, onClose }) => {
  ...
  return (
    <div className="modalOverlay">
      <div 
        className={`${modalContent} ${isInView ? backgroundLoaded : ''}`}
        ref={formRef}
      >
        <span className="closeBtnContact" onClick={onClose}>&times;</span>
        
        <form className="contactForm" onSubmit={handleSubmit}>
          {formData.fields
            .filter(field => field.name !== 'message')
            .map((field, index) => (
              <div className="contactFormGroup" key={index}>
          ...
          <button type="submit" className="submitBtnContact">{buttonText}</button>
        </form>
